...
<script>
  ...
  export let config;
  export let raw;
  // export let _skeleton

  let _this;
  let last_config;

  // $: console.log('fg config', config.type, config);
  // $: console.log('fg values', config.type, values);
  ...
  function state_value (s) {
    // TODO integrate with state as property. use serde:[de]serialize everywhere
  ...
  $: config = Object.assign({'type': 'form'}, config || {});
